...
/**
 * A DataTable object can be used to store data that shall be written to a csv
 * file. Data will be added column wise. The size of the columns may differ.
 * When writing to csv shorter columns will be filled by an defined (NA) value.
 * 
 * @author sfuerst
 *
 */
...
public class DataTable extends ArrayList<ArrayList<String>> {
    ...
    /**
     * 
     * @return the number of rows without header. This is the maximal size of
     *         all columns.
     */
     ...
     * 
     * @param naValue,
     *            the String used for not assigned values
     * @param withHeader
     * @param pathToFile
     * @throws IOException
     */
    @SuppressWarnings("resource")
    public void writeToCsv(String naValue, boolean withHeader,
            String pathToFile) throws IOException {
        int numberOfColumns = this.size();
        if (header.size() != numberOfColumns) {
            throw new IOException("Header " + header
                    + " do not fit to the number of columns [" + numberOfColumns
                    + "] in this Table.");
        }
        pathToFile = FileWriterUtils.checkFilePath(pathToFile,
                FileWriterUtils.CSV_EXTENSION);
    ...
    /**
     * inserts this map as two columns (keyColumn and value column) at the
    ...
    /**
     * Creates a column with all entries equal to the given value. This method
     * should only be used if the total row number of this table will not change
     * anymore. Otherwise the created column will miss some entries.
     * 
     * @param value
     */
    ...
    /**
     * Creates a column with all entries equal to the given value and inserts it
     * at the given colzmnIndex. This method should only be used if the total
     * row number of this table will not change anymore. Otherwise the created
     * column will miss some entries.
     * 
     * @param value
     */
    ...
    public String toString(String naValue, boolean withHeader) {
        ...
        for (int index = 0; index < numberOfColumns; index++) {
            ...
            for (int row = 0; row < this.get(index).size(); row++) {
                maxCharLength = Math.max(maxCharLength,
                        this.get(index).get(row).length());
            }
            maximalCharacterLengthPerColumn[index] = maxCharLength;
        }
        for (String headline : header) {
            int maxCharLength = maximalCharacterLengthPerColumn[header
                    .indexOf(headline)];
            tableStringBuffer.append(
                    String.format("%" + maxCharLength + "s | ", headline));
        }
        tableStringBuffer.append("\n");
        ...
        for (int row = 0; row < numberOfRows; row++) {
            for (int column = 0; column < numberOfColumns; column++) {
                ...
                try {
                    tableStringBuffer
                            .append(String.format("%" + maxCharLength + "s | ",
                                    this.get(column).get(row)));
                } catch (IndexOutOfBoundsException e) {
                    tableStringBuffer.append(String
                            .format("%" + maxCharLength + "s | ", naValue));
                }
            }
            tableStringBuffer.append("\n");
        }

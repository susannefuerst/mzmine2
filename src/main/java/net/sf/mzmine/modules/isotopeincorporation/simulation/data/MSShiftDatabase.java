...
public class MSShiftDatabase extends MSDatabase {
    ...
    public SimulatedSpectrumDataset toSimulatedSpectrumDataSet(IncorporationType incType) {
        MassSpectrum spectrum = this.getSpectrum(incType);
        LabeledSimpleDataPoint[] dataPoints = new LabeledSimpleDataPoint[spectrum.size()];
        int entryCount = 0;
        for (Entry<Double, Double> entry : spectrum.entrySet()) {
            String isotopesString = "Heavy isotopes: " + this.shiftInducingIsotopes(incType, entry.getKey());
            LabeledSimpleDataPoint datapoint = new LabeledSimpleDataPoint(entry.getKey(), entry.getValue(), isotopesString);
            dataPoints[entryCount] = datapoint;
            entryCount++;
        }
        String label = "Simulation of " + getIncorporatedTracers() + " incorporation in " + getFragmentKey().name() + ", " + getFragmentFormula();

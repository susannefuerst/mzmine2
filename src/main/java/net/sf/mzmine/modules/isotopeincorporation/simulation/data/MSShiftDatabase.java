...
public class MSShiftDatabase extends MSDatabase {
    ...
    public static final MyLogger LOGGER = MyLogger
            .getLogger(MSShiftDatabase.class);
    ...
    public void parseCsv(String absoluteFilePath) {
        ...
        try {
            ...
            setNaturalSpectrum(ParserUtils.parseSpectrum(records,
                    ...
                    MSShiftDatabaseColKey.NATURAL_FREQUENCY.getColumnIndex(),
                    FrequencyType.MID, 1));
            setMarkedSpectrum(ParserUtils.parseSpectrum(records,
                    MSShiftDatabaseColKey.MARKED_MASS.getColumnIndex(),
                    MSShiftDatabaseColKey.MARKED_FREQUENCY.getColumnIndex(),
                    FrequencyType.MID, 1));
            setMixedSpectrum(ParserUtils.parseSpectrum(records,
                    MSShiftDatabaseColKey.MIXED_MASS.getColumnIndex(),
                    MSShiftDatabaseColKey.MIXED_FREQUENCY.getColumnIndex(),
                    FrequencyType.MID, 1));
            setIncorporationRate(Double.parseDouble(records.get(1)
                    .get(MSShiftDatabaseColKey.INC_RATE.getColumnIndex())));
            setFragmentKey(FragmentKey.byKeyName(records.get(1)
                    .get(MSShiftDatabaseColKey.FRAGMENT_KEY.getColumnIndex())));
            setIncorporatedTracers(records.get(1)
                    .get(MSShiftDatabaseColKey.INCORPORATED_TRACERS
                            .getColumnIndex()));
    ...
    public void writeCsv(String outputFolderPath) throws IOException {
        ...
        DataTable dataTable = new DataTable(
                MSShiftDatabaseColKey.toHeaderList());
    ...
    public String createFilename() {
        ...
        return getFragmentKey() + "_" + getIncorporatedTracers() + "_"
                + incorporationPerCent + "_shift";
    }
    ...
    public void analyseAllShifts() {
        ...
        if (getNaturalSpectrum() != null) {
            naturalMassShifts = getNaturalSpectrum()
                    .analyseMassShifts(elements);
            LOGGER.debug("Analysed naturalMassShifts");
        } else {
            LOGGER.warn(
                    "Missing naturalSpectrum to determine naturalMassShifts");
        }
    ...
    /**
     * Creates a nice formula representation of the isotopes that induced the
     * shift from the p_0 peak to the peak with the parameter mass. i.e.
     * (¹²C)₂(¹³C)₃(¹H)₂(²H)₅(¹⁵N)₂
     * 
     * @param incType
     *            a hint to the MassSpectrum we refer to
     *            IncorporationType.NATURAL -> natural spectrum
     *            IncorporationType.MARKED -> marked spectrum
     *            IncorporationType.MIXED -> mixed spectrum
     * @param mass
     * @return A nice formula representation of the isotopes that induce this
     *         mass, i.e. (¹²C)₂(¹³C)₃(¹H)₂(²H)₅(¹⁵N)₂.
     */
    public String shiftInducingIsotopesNiceFormatted(IncorporationType incType,
            Double mass) {
     ...
     * @param mass
     * @return A nice formula representation of the isotopes that induce this
     *         mass, i.e. (12C)2(13C)3(1H)2(2H)5(15N)2.
     */
    public String shiftInducingIsotopesSimpleString(IncorporationType incType,
            Double mass) {
        return shiftInducingIsotopes(incType, mass).toSimpleString();
    }

    public IsotopeFormula shiftInducingIsotopes(IncorporationType incType,
            Double mass) {
        MassSpectrum spectrum = spectrumByIncorporationType(incType);
        List<Entry<Double, Double>> spectrumEntryList = new ArrayList<>(
                spectrum.entrySet());
        MassShiftDataSet shiftDataset = shiftDatasetByIncorporationType(
                incType);
        List<Entry<MassShiftList, IsotopeListList>> shiftEntryList = new ArrayList<>(
                shiftDataset.entrySet());
        int massIndex = 0;
        ...
        IsotopeListList shiftInducingIsotopes = shiftEntryList.get(massIndex)
                .getValue();
    ...
    /**
     * @param incType
     *            a hint to the MassSpectrum or MassShiftDataset we refer to:
     *            IncorporationType.NATURAL -> natural spectrum
     *            IncorporationType.MARKED -> marked spectrum
     *            IncorporationType.MIXED -> mixed spectrum
     * @return The MassShiftDataset related to this IncorporationType
     */
    public MassShiftDataSet shiftDatasetByIncorporationType(

...
class CustomDBSearchTask extends AbstractTask {
    ...
    private Integer idIndex, mzIndex, rtIndex, identityIndex, formulaIndex,
            riIndex;
    ...
    CustomDBSearchTask(PeakList peakList, ParameterSet parameters) {
        ...
        idIndex = parameters.getParameter(CustomDBSearchParameters.ID)
                .getValue();
        mzIndex = parameters.getParameter(CustomDBSearchParameters.MZ)
                .getValue();
        rtIndex = parameters.getParameter(CustomDBSearchParameters.RT)
                .getValue();
        identityIndex = parameters
                .getParameter(CustomDBSearchParameters.IDENTITY).getValue();
        formulaIndex = parameters.getParameter(CustomDBSearchParameters.FORMULA)
                .getValue();
        riIndex = parameters.getParameter(CustomDBSearchParameters.RI)
                .getValue();
    ...
    private void processOneLine(String values[]) {
        ...
        double lineMZ = 0, lineRT = 0, lineRI = 0;
        lineID = values[idIndex];
        lineName = values[identityIndex];
        lineFormula = values[formulaIndex];
        lineMZ = Double.parseDouble(values[mzIndex]);
        lineRT = Double.parseDouble(values[rtIndex]);
        if (riIndex != null) {
            lineRI = Double.parseDouble(values[riIndex]);
        }

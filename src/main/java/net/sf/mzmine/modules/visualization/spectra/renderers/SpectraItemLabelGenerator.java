...
public class SpectraItemLabelGenerator implements XYItemLabelGenerator {
     ...
     * not overlap
     */
    public static final int POINTS_RESERVE_X = 80;
    ...
    public String generateLabel(XYDataset dataset, int series, int item) {
        boolean log = false;
        if (dataset instanceof ComparingSpectrumDataset) {
            String test = "test";
            log = true;
            // double mzValue = dataset.getXValue(series, item);
            // return mzFormat.format(mzValue);
        }
        ...
        for (int i = 1; (item - i > 0) || (item + i < itemCount); i++) {
            ...
            if ((item - i > 0)
                ...
                if (log) {
                    logger.info("skipped label for [" + originalX + ", "
                            + originalY + "] --> detected higher datapoint: ["
                            + dataset.getXValue(series, item - i) + ", "
                            + dataset.getYValue(series, item - i) + "]");
                }
                return null;
            }

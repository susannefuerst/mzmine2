...
public class IsotopeSetTest extends TestCase {
    ...
    private void assertEqualsSpectra(MassSpectrum expected, MassSpectrum actual, double allowedFrequencyError) {
        for (Entry<Double, Double> expectedEntry : expected.entrySet()) {
            if (actual.get(expectedEntry.getKey()) == null && expectedEntry.getValue() <= allowedFrequencyError) {
                actual.put(expectedEntry.getKey(), 0.0);
            }
            Double frequencyDifference = Math.abs(expectedEntry.getValue() - actual.get(expectedEntry.getKey()));
            assertTrue(frequencyDifference + "is not smaller than " + allowedFrequencyError, frequencyDifference <= allowedFrequencyError);
        }
        for (Entry<Double, Double> actualEntry : actual.entrySet()) {
            if (expected.get(actualEntry.getKey()) == null && actualEntry.getValue() <= allowedFrequencyError) {
                expected.put(actualEntry.getKey(), 0.0);
            }
            assertTrue(Math.abs(actualEntry.getValue() - expected.get(actualEntry.getKey())) <= allowedFrequencyError);
        }
    }
    ...
    /*
     * compare simulated spectra for E_n fragments (where E is any element and n a natural number with range defined in the test)
